...
// ── Typedefs matching synheart-core-runtime edge_ffi.rs ────────────
...
typedef _PushRrC =
    Void Function(Pointer<Void>, Int64, Double);
...
class _Edge {
  ...
  late final lastSleepScore =
      _lib.lookupFunction<_JsonReturnHandleC, _JsonReturnHandleDart>(
          'synheart_core_edge_last_sleep_score_json');
  late final wearableRef =
      _lib.lookupFunction<_JsonReturnHandleC, _JsonReturnHandleDart>(
          'synheart_core_edge_wearable_reference_json');
  late final exportSnap =
      _lib.lookupFunction<_JsonReturnHandleC, _JsonReturnHandleDart>(
...
String? _readAndFree(_Edge e, Pointer<Utf8> p) {
  ...
  // RFC-Physio-002 doc title). `toDartString` does strict UTF-8
...
void main() {
  ...
  if (lib == null) {
    test('skipped — native dylib not found', () {
      // Rebuild with: cargo build -p synheart-core-runtime --release
      //   --features edge (in synheart-core-runtime), then copy
      //   target/release/libsynheart_core_runtime.dylib into
      //   synheart-core-flutter/native/.
    }, skip: 'libsynheart_core_runtime.dylib not present');
    return;
  }
  ...
  group('E2E edge FFI sleep score', () {
    ...
    test('attach + tick yields sleep_score axis in next HSI', () {
      ...
      try {
        ...
        final phys =
            (v['axes'] as Map)['physiological'] as List<Object?>;
        final names = phys.cast<Map<String, Object?>>().map((r) => r['name']).toList();
