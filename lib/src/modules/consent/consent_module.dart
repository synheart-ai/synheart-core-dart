...
class ConsentModule extends BaseSynheartModule implements ConsentProvider {
  ...
  final ConsentConfig? _consentConfig;
  ...
  Future<void> updateConsentType(ConsentType type, bool granted) async {
    ...
    final updated = _currentConsent!.copyWith(
      ...
      phoneContext: type == ConsentType.phoneContext
          ? granted
          : _currentConsent!.phoneContext,
  ...
  void _logConsentChanges(
    ...
    for (final e in fields.entries) {
      if (e.value.$1 != e.value.$2) {
        SynheartLogger.log(
          'Consent changed: ${e.key} ${e.value.$2 ? "granted" : "revoked"}',
        );
      }
    }
  }
  ...
  Future<List<ConsentProfile>> getAvailableProfiles() async {
    ...
    SynheartLogger.log(
      '[ConsentModule] getAvailableProfiles: returning empty list',
    );
    return [];
  }
  ...
  ///
  /// [grantedChannels] and [tier] enable granular consent.
