...
//
// Mirrors the JSON shapes produced by `synheart_sleep_score` (Rust, in
// synheart-engine-runtime). Every field name matches the snake_case key
// used by serde on the Rust side. See
// `synheart-engine-runtime/docs/SLEEP-SCORE-INTEGRATION.md` and
// `crates/sleep-score/README.md` for semantics.

import 'dart:convert';

/// Canonical sleep stage. See `SleepStage` in the Rust crate.
enum SleepStage {
  awake('awake'),
  light('light'),
  deep('deep'),
  ...
  static SleepStage fromWire(String s) =>
...
enum SleepPath {
  ...
  static SleepPath fromWire(String s) =>
      SleepPath.values.firstWhere(
        (v) => v.wire == s,
...
enum SleepScoreMode {
  ...
  static SleepScoreMode fromWire(String s) =>
      SleepScoreMode.values.firstWhere(
        (v) => v.wire == s,
...
/// The three vendor-input shapes. Rust `NightInput` is a tagged enum
...
class SleepScoreInput {
  ...
  Map<String, Object?> toJson() => {
        ...
        'priors_newest_first':
            priorsNewestFirst.map((p) => p.toJson()).toList(),
...
class SleepScoreResult {
  ...
  factory SleepScoreResult.fromJson(Map<String, Object?> json) =>
      SleepScoreResult(
        ...
        mode: SleepScoreMode.fromWire(
            json['mode'] as String? ?? 'cold_start'),
...
class WearableReferenceView {
  ...
  factory WearableReferenceView.fromJsonString(String s) =>
      WearableReferenceView.fromJson(
