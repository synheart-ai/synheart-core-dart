...
class CoreRuntimeBridge {
 ...
 _withCString(dimension, (p) =>
 _ffi.srmPushWearableDaily(_handle, p, dayIndex, value, confidence, fidelity));
 }
 ...
 /// 2=Full`.
 void srmTriggerWearableRecompute({int triggerType = 0, required int asOfDay}) =>
 ...
 String inputJson, String correlationId) {
 return _withCString(inputJson, (p) {
 return _withCString(correlationId, (cid) {
 return _readAndFree(
 ...
 try {
 return _readAndFree(
 fn(_handle, pDev.cast(), pPlat.cast(), pOs.cast(), pUser.cast(), pExtra.cast()),
 );
 } finally {
 malloc.free(pDev);
 malloc.free(pPlat);
