...
Pod::Spec.new do |s|
  ...
  # Force-load the Rust static library to preserve synheart_core_* symbols
  # for Dart FFI's dlsym (DynamicLibrary.process on iOS).
  #
  # The Rust archive is built with onnx-system feature (ort/disable-linking)
  # so it contains NO bundled ONNX Runtime C++ code — safe to force-load.
  # ONNX Runtime is provided by the onnxruntime-c pod below.
  ...
  # ONNX Runtime C API — linked separately from the Rust archive to avoid
  # duplicate symbols from ort-sys's merged static libraries (protobuf,
  # abseil, re2 share C++ template instantiations across compilation units).
