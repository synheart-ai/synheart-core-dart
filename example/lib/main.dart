...
class _SynheartExamplePageState extends State<SynheartExamplePage> {
  ...
  sb.BehaviorSession? _currentBehaviorSession;
  List<sb.BehaviorEvent> _sessionEvents = [];
  StreamSubscription<sb.BehaviorEvent>? _behaviorEventSubscription;
    ...
    try {
      ...
      if (behaviorModule?.synheartBehavior != null) {
        try {
          ...
          await _requestBehaviorPermissions(behaviorModule!.synheartBehavior!);

          // Start session and track it
          final session = await behaviorModule.synheartBehavior!.startSession();
          setState(() {
            _currentBehaviorSession = session;
            _sessionEvents = []; // Clear previous session events
          });

          // Listen to behavior events for this session
          _behaviorEventSubscription = behaviorModule.synheartBehavior!.onEvent
              .listen((event) {
                if (_currentBehaviorSession != null &&
                    event.sessionId == _currentBehaviorSession!.sessionId) {
                  setState(() {
  ...
  Future<void> _stopSynheart() async {
    ...
    final behaviorModule = Synheart.shared.behaviorModule;
    if (_currentBehaviorSession != null &&
        behaviorModule?.synheartBehavior != null) {
      try {
        // Cancel event subscription
        await _behaviorEventSubscription?.cancel();
        _behaviorEventSubscription = null;

        // End the session and get summary
        final summary = await _currentBehaviorSession!.end().timeout(
        ...
        // Store events for navigation
        final sessionEvents = List<sb.BehaviorEvent>.from(_sessionEvents);

        // Clear session state
        setState(() {
          _currentBehaviorSession = null;
          _sessionEvents = [];
        });
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => SessionResultsScreen(
                ...
                behavior: behaviorModule!.synheartBehavior,
              ),
            ),
          );
        }
        ...
        // Continue with stopping Synheart even if session end failed
      }
    }
  ...
  /// Request behavior-related permissions (call and notification)
  Future<void> _requestBehaviorPermissions(sb.SynheartBehavior behavior) async {
    try {
      // Request call permission
      final hasCallPermission = await behavior.checkCallPermission();
      if (!hasCallPermission) {
        await behavior.requestCallPermission();
      }

      // Request notification permission
      final hasNotificationPermission = await behavior
          .checkNotificationPermission();
      if (!hasNotificationPermission) {
        await behavior.requestNotificationPermission();
      }
    } catch (e) {
      // Permission requests are optional, log but don't fail initialization
      print('Note: Could not request behavior permissions: $e');
    }
  }
