...
class SynheartProvider extends ChangeNotifier {
  ...
  bool get isSessionRunning => _isInitialized && Synheart.isSessionRunning;
    ...
    try {
      ...
      if (affect != null) {
        for (final r in affect) {
          ...
          if (m['name'] == 'valence')
            valence = (m['value'] as num?)?.toDouble();
          if (m['name'] == 'arousal')
            arousal = (m['value'] as num?)?.toDouble();
        }
      }
      ...
      if (engagement != null) {
        for (final r in engagement) {
          ...
          if (m['name'] == 'capacity')
            capacity = (m['value'] as num?)?.toDouble();
        }
      }
      return {
        'valence': valence,
  ...
  Future<void> initialize({
    ...
    try {
      ...
      // See example/SETUP.md for instructions on how to obtain these credentials.
      final finalConfig =
          config ??
          SynheartConfig(
            ...
            // CloudConfig intentionally omitted in the demo — uploads
            // require platform credentials. Wire one in here when you
            // have an app registered at platform.synheart.ai.
          );
  ...
  void _throttledNotify() {
    ...
    _notifyThrottleTimer = Timer(
      const Duration(milliseconds: _notifyThrottleMs),
      () {
  ...
  Future<void> saveConsentCredentials({
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_keyConsentServiceUrl, consentServiceUrl.trim());
    ...
    _savedConsentAppId = appId.trim().isEmpty ? null : appId.trim();
    _savedConsentAppApiKey = appApiKey.trim().isEmpty ? null : appApiKey.trim();
    notifyListeners();
  }
    ...
    try {
      ...
      _sessionProgressTimer = Timer.periodic(const Duration(seconds: 1), (_) {
        ...
        if (_sessionStartTime != null) {
          _sessionElapsedSec = DateTime.now()
  ...
  // Per-module on-demand feature queries are no longer part of the public
  // SDK surface. Subscribe to `Synheart.onHSIUpdate` (raw JSON) or
  // `Synheart.onStateUpdate` (typed) for per-window state instead.
  Future<void> queryWearFeatures() async {
    _queriedFeatures = {
      'info': 'Subscribe to Synheart.onHSIUpdate for per-window state.',
    };
    notifyListeners();
  }
  ...
  void _handleWatchSessionEvent(SessionEvent event) {
    switch (event) {
      ...
      case SessionFrame():
        ...
        final sdnn =
            (metrics['sdnn_ms'] as num?)?.toDouble() ??
